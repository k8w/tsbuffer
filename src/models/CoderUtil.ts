...
export class CoderUtil {
  static isJsonCompatible(
    ...
    type: "encode" | "decode",
    protoHelper: TSBufferValidator["protoHelper"]
  ): boolean {
    let schemaInfo: TSBufferSchema & { isJsonEncodable?: boolean; isJsonDecodable?: boolean } =
      schema
    ...
    if (schemaInfo[key] === undefined) {
      ...
      switch (schema.type) {
        ...
        case SchemaType.Tuple:
          schemaInfo[key] = schema.elementTypes.every(v =>
        ...
        case SchemaType.Interface:
          ...
          if (flatSchema.indexSignature) {
            schemaInfo[key] =
              schemaInfo[key] &&
              this.isJsonCompatible(flatSchema.indexSignature.type, type, protoHelper)
          }
          break
        ...
        case SchemaType.Intersection:
          schemaInfo[key] = schema.members.every(v =>
            this.isJsonCompatible(v.type, type, protoHelper)
          )
          break
        ...
        case SchemaType.Overwrite: {
          ...
          if (parsed.type === SchemaType.Interface) {
            ...
            schemaInfo[key] = parsed.members.every(v =>
              this.isJsonCompatible(v.type, type, protoHelper)
            )
          }
          break
