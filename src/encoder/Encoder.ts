...
export class Encoder {
  ...
  private _writeInterface(
    ...
    if (schema.extends) {
      ...
      for (let extend of schema.extends) {
        ...
        // extend Block
        let parsedExtend = this._validator.protoHelper.parseReference(
          extend.type
    ...
    this._writer.ops.splice(
      opStartOps,
      0,
      this._writer.req2op({ type: "varint", value: Varint64.from(blockIdCount) })
    )
  }
  ...
  private _parseOverwrite(value: any, schema: OverwriteTypeSchema) {
    ...
    // 解引用
    let target = this._validator.protoHelper.parseReference(schema.target) as Exclude<
      OverwriteTypeSchema["target"],
      TypeReference
    >
    let overwrite = this._validator.protoHelper.parseReference(schema.overwrite) as Exclude<
      OverwriteTypeSchema["overwrite"],
      TypeReference
    >
