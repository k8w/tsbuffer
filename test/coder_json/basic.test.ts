...
describe("Basic Encode", function () {
  ...
  it("Any", function () {
    ...
    ;[0, 123, "abc", true, null, { a: 1, b: "测试", c: [1, 2, 3] }, [1, 2, "3a", "adf3"]].forEach(
      v => {
  ...
  it("Buffer", function () {
    ...
    for (let key in typedArrays) {
      ...
      assert.deepStrictEqual(
        tsb.decodeJSON(JSON.parse(JSON.stringify(tsb.encodeJSON(arr1, "a/b").json!)), "a/b").value,
        arr1,
        key + " failed"
      )
      ...
      // subarray 测试
      assert.deepStrictEqual(
        tsb.decodeJSON(JSON.parse(JSON.stringify(tsb.encodeJSON(arr, "a/b").json!)), "a/b").value,
        arr,
        key + " failed"
      )
    }
