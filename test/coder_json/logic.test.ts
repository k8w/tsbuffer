...
describe("LogicTypes", function () {
  it("A | B", async function () {
    ...
    assert.deepStrictEqual(
      tsb.decodeJSON(JSON.parse(JSON.stringify(tsb.encodeJSON(null, "a/b2").json!)), "a/b2").value,
      null
    )
    assert.deepStrictEqual(
      tsb.decodeJSON(JSON.parse(JSON.stringify(tsb.encodeJSON([null], "a/arr").json!)), "a/arr")
        .value,
      [null]
    )
  })
  ...
  it("Unknown Union ID", async function () {
    ...
    ;[{ a: "a" }, { b: 12 }, { c: true }, { a: "asdg", b: 12412, c: false }].forEach(v => {
      assert.deepStrictEqual(
        tsb.decodeJSON(JSON.parse(JSON.stringify(tsb.encodeJSON(v, "a/a").json!)), "a/a1").value,
        v
      )

      let v1 = Object.assign({}, v, { d: [true, false] })
      assert.deepStrictEqual(
        tsb.decodeJSON(JSON.parse(JSON.stringify(tsb.encodeJSON(v1, "a/a1").json!)), "a/a").value,
        v
      )
    })
  })
  ...
  it("Unknown Intersection ID", async function () {
    ...
    assert.deepStrictEqual(
      tsb.decodeJSON(JSON.parse(JSON.stringify(tsb.encodeJSON(v, "a/a1").json!)), "a/a").value,
      v
    )
    assert.deepStrictEqual(
      tsb.decodeJSON(JSON.parse(JSON.stringify(tsb.encodeJSON(v1, "a/a1").json!)), "a/a").value,
  ...
  it("Omit & {a?: string}", async function () {
    ...
    assert.deepStrictEqual(
      tsb.decodeJSON(tsb.encodeJSON({ a: "xxx", b: 123 }, "a/b").json!, "a/b").value,
      tsb.decodeJSON(tsb.encodeJSON({ a: "xxx", b: 123 }, "a/b1").json!, "a/b1").value
    )
  })
})
