...
export type Overwrite1 = Overwrite<
  Base,
  {
    sex: 'm' | 'f';
    other: string;
...
>;
export type Overwrite2 = Overwrite<
  Pick<Base, 'name' | 'sex'>,
  {
    sex: 'm' | 'f';
    other: string;
...
export type IOverwrite1 = Overwrite<
  Base1,
  {
    a: number;
    b: string;
...
>;
export type IOverwrite2 = Overwrite<
  Base1,
  {
    b: string;
    [key: string]: string;
...
// only common
export type NestedAB = Pick<
  Omit<Pick<AB, 'type' | 'common' | 'common1'>, 'common1' | 'type'>,
  'common'
>;
...
export type PickIntersection1 = Pick<
  { a: string; b: string } & { c: string; d: string },
  'a' | 'c'
>;
export type OmitIntersection1 = Omit<
  { a: string; b: string } & { c: string; d: string },
  'b' | 'd'
>;
export type PickIntersection2 = Pick<
  (
    | { type: 'a'; value: { a: string } }
    | { type: 'b'; value: { b: string } }
  ) & { meta: string },
  'type' | 'value'
>;
export type OmitIntersection2 = Omit<
  (
    | { type: 'a'; value: { a: string } }
    | { type: 'b'; value: { b: string } }
  ) & { meta: string },
