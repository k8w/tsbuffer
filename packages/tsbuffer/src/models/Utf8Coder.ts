    ...
    for (let i = pos, end = pos + length; i < end; ) {
      ...
      if (t <= 0x7f) {
        ...
        str += String.fromCharCode(
          ((t & 0xf) << 12) | ((buf[i++] & 0x3f) << 6) | (buf[i++] & 0x3f)
        );
      } else if (t >= 0xf0) {
        const t2 =
          (((t & 7) << 18) |
...
const Utf8CoderNode: IUtf8Coder = {
  ...
  write: (str: string, buf: Uint8Array, pos: number) =>
    Buffer.from(buf.buffer, buf.byteOffset, buf.byteLength).write(
      str,
      pos,
      'utf-8'
    ),
  read: (buf: Uint8Array, pos: number, length: number) =>
    Buffer.from(buf.buffer, buf.byteOffset, buf.byteLength).toString(
      'utf-8',
      pos,
      pos + length
    ),
