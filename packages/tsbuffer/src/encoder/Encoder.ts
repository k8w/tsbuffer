...
export class Encoder {
  ...
  encodeJSON(value: any, schema: TSBufferSchema): any {
    ...
    if (
      typeof value !== 'object' ||
      value === null ||
      CoderUtil.isJsonCompatible(schema, 'encode', this._validator.protoHelper)
    ) {
      return value;
    }
    ...
    switch (schema.type) {
      case SchemaType.Array:
        ...
        return (value as any[]).map((v) =>
          this.encodeJSON(v, schema.elementType)
        );
      case SchemaType.Tuple: {
        ...
        return (value as any[]).map((v, i) =>
          this.encodeJSON(v, schema.elementTypes[i])
        );
      }
      case SchemaType.Interface: {
        ...
        const flatSchema =
          this._validator.protoHelper.getFlatInterfaceSchema(schema);
        for (const key in value) {
          ...
          if (property) {
            ...
            value[key] = this.encodeJSON(
              value[key],
              flatSchema.indexSignature.type
            );
          }
        }
        return value;
      ...
      case SchemaType.Buffer:
        ...
        if (schema.arrayType) {
          ...
          return Base64Util.bufferToBase64(
      ...
      case SchemaType.Keyof:
        return this.encodeJSON(
          value,
          this._validator.protoHelper.parseReference(schema)
        );
    ...
    switch (schema.type) {
      ...
      case SchemaType.Tuple: {
        if (schema.elementTypes.length > 64) {
          throw new Error(
            'Elements oversized, maximum supported tuple elements is 64, now get ' +
        ...
        for (let i = 0; i < _v.length; ++i) {
          ...
          if (
            _v[i] === undefined ||
            this._nullAsUndefined(_v[i], schema.elementTypes[i])
          ) {
            continue;
          }
      ...
      case SchemaType.Date:
        this._writer.push({
          type: 'varint',
          value: Varint64.from((value as Date).getTime()),
        });
        break;
      case SchemaType.NonNullable:
  ...
  private _writePureMappedType(
    value: any,
    schema: MappedTypeSchema,
    options?: {
      skipFields?: { [fieldName: string]: 1 };
      pickFields?: { [fieldName: string]: 1 };
    }
    ...
    // Write Interface
    const parsedTarget = this._validator.protoHelper.parseReference(
      schema.target
    );
    if (parsedTarget.type === 'Interface') {
      this._writeInterface(value, parsedTarget, options);
    } else {
      this._writePureMappedType(
        value,
        parsedTarget as MappedTypeSchema,
        options
      );
    }
  }
  ...
  private _writeInterface(
    ...
    if (schema.extends) {
      ...
      if (schema.extends.length > Config.interface.maxExtendsNum) {
        throw new Error(
          `Max support ${Config.interface.maxExtendsNum} extends, actual: ${schema.extends.length}`
        );
      }
      ...
      for (const extend of schema.extends) {
        ...
        // extend Block
        const parsedExtend = this._validator.protoHelper.parseReference(
          extend.type
        ) as InterfaceTypeSchema;
        this._writeInterface(value, parsedExtend, {
          ...options,
          // 确保indexSignature是在最小层级编码
          skipIndexSignature:
            !!schema.indexSignature || options.skipIndexSignature, // 如果父级有indexSignature 或 父级跳过 则跳过indexSignature
        });
    ...
    if (schema.properties) {
      for (const property of schema.properties) {
        const parsedType = this._validator.protoHelper.parseReference(
          property.type
        );
    ...
    if (!options.skipIndexSignature) {
      ...
      if (flat.indexSignature) {
        for (const key in value) {
          if (
            value[key] === undefined ||
            this._nullAsUndefined(value[key], flat.indexSignature.type)
          ) {
            continue;
          }
  ...
  private _parseOverwrite(value: any, schema: OverwriteTypeSchema) {
    ...
    // 解引用
    const target = this._validator.protoHelper.parseReference(
      schema.target
    ) as Exclude<OverwriteTypeSchema['target'], TypeReference>;
    const overwrite = this._validator.protoHelper.parseReference(
      schema.overwrite
    ) as Exclude<OverwriteTypeSchema['overwrite'], TypeReference>;
    const flatTarget =
      this._validator.protoHelper.getFlatInterfaceSchema(target);
    const flatOverwrite =
      this._validator.protoHelper.getFlatInterfaceSchema(overwrite);
  ...
  private _writeUnion(
    ...
    if (idNum > 0) {
      ...
      this._writer.ops.splice(
        encodeStartPos,
        0,
        this._writer.req2op({ type: 'varint', value: Varint64.from(idNum) })
      );
      return;
    } else {
      // 未编码，没有任何条件满足，抛出异常
  ...
  private _writeBuffer(value: ArrayBuffer | TypedArray) {
    ...
    // 其它TypedArray 转为Uint8Array
      ...
      const uint8Arr = new Uint8Array(
        value.buffer,
        value.byteOffset,
        value.length * arrType.BYTES_PER_ELEMENT
      );
   ...
   * @param idPos
   */
  private _processIdWithLengthType(
    idPos: number,
    payloadType: TSBufferSchema,
    lengthPrefixPos?: number
  ) {
    ...
    // 解引用
    const parsedSchema =
      this._validator.protoHelper.parseReference(payloadType);

    const lengthInfo = IdBlockUtil.getPayloadLengthInfo(
      parsedSchema,
      this._validator.protoHelper
    );
    ...
    if (lengthInfo.needLengthPrefix) {
      const payloadByteLength = this._writer.ops
        .filter((v, i) => i > idPos)
        .sum((v) => v.length);
      this._writer.ops.splice(
        lengthPrefixPos == undefined ? idPos + 1 : lengthPrefixPos,
        0,
        this._writer.req2op({
          type: 'varint',
