...
export class Decoder {
  ...
  decodeJSON(json: any, schema: TSBufferSchema): any {
    if (
      json === null ||
      CoderUtil.isJsonCompatible(schema, 'decode', this._validator.protoHelper)
    ) {
      return json;
    }
    ...
    switch (schema.type) {
      case SchemaType.Array:
        ...
        return (json as any[]).map((v) =>
          this.decodeJSON(v, schema.elementType)
        );
  ...
  private _read(schema: TSBufferSchema): unknown {
    switch (schema.type) {
      ...
      case SchemaType.Tuple: {
        ...
        for (
          let i = 0, nextMaskIndex = 0, next = maskIndices[0];
          i <= maxIndex;
          ++i
        ) {
          if (i === next) {
  ...
  private _readPureMappedType(
    ...
    if (parsedTarget.type === 'Interface') {
      ...
      parsedTarget.type === 'Pick' ||
      parsedTarget.type === 'Omit' ||
      parsedTarget.type === 'Partial' ||
      parsedTarget.type === 'Overwrite'
    ) {
    ...
    for (let i = 0; i < blockIdNum; ++i) {
      ...
      if (blockId === 0) {
        if (flatSchema.indexSignature) {
          ...
          this._skipIdLengthPrefix(
            this._validator.protoHelper.parseReference(type)
          );
      ...
      // extend block
        ...
        const extend =
          schema.extends && schema.extends.find((v) => v.id === extendId);
        if (extend) {
          this._skipIdLengthPrefix(
            this._validator.protoHelper.parseReference(extend.type)
          );
      ...
      // property
        ...
        const property =
          schema.properties &&
          schema.properties.find((v) => v.id === propertyId);
        if (property) {
          this._skipIdLengthPrefix(
            this._validator.protoHelper.parseReference(property.type)
          );
  ...
  private _undefinedAsNull(
    ...
    return (
      ...
      !SchemaUtil.canBeLiteral(type, undefined) &&
      !isOptional &&
      SchemaUtil.canBeLiteral(type, null)
    );
  }

  private _skipIdLengthPrefix(
  ...
  private _readUnionOrIntersection(
    ...
    for (let i = 0; i < idNum; ++i) {
      ...
      this._skipIdLengthPrefix(
        this._validator.protoHelper.parseReference(member.type)
      );
  ...
  private _getBufferValue(
    uint8Arr: Uint8Array,
    schema: BufferTypeSchema
  ): ArrayBuffer | ArrayBufferView {
    if (schema.arrayType) {
      if (
        schema.arrayType === 'BigInt64Array' ||
        schema.arrayType === 'BigUint64Array'
      ) {
      ...
      // 其余TypedArray 可能需要内存拷贝 性能次之
        ...
        if (uint8Arr.byteOffset % typedArr.BYTES_PER_ELEMENT === 0) {
          return new typedArr(
            uint8Arr.buffer,
            uint8Arr.byteOffset,
            uint8Arr.byteLength / typedArr.BYTES_PER_ELEMENT
          );
        }
        // 字节不对齐，不能直接转，只能拷贝内存后再生成
        else {
          const arrBuf = uint8Arr.buffer.slice(
            uint8Arr.byteOffset,
      ...
      return uint8Arr.byteLength === uint8Arr.buffer.byteLength &&
        uint8Arr.byteOffset === 0
        ? uint8Arr.buffer
        : uint8Arr.buffer.slice(
            uint8Arr.byteOffset,
            uint8Arr.byteOffset + uint8Arr.byteLength
          );
    }
