...
describe('Interface', function () {
  it('property', async function () {
    ...
    assert.equal(
      tsb.encode({ a: '哈哈' }, 'a/b', { skipValidate: true }).buf!.length,
      9
    );
    ...
    assert.equal(
      tsb.encode({ a: '哈哈', e: { a: '0'.repeat(1000), b: '哈哈' } }, 'a/b')
        .buf!.length,
      21
    );
    ...
    [
      ...
      assert.deepStrictEqual(
        tsb.decode(tsb.encode(v, 'a/b').buf!, 'a/b').value,
        v
      );
    });
    assert.deepStrictEqual(
      tsb.decode(tsb.encode({ a: 'xxxx', b: undefined }, 'a/b').buf!, 'a/b')
        .value,
      { a: 'xxxx' }
    );
  });
  ...
  it('extends', async function () {
    ...
    assert.equal(
      tsb.encode({ v1: 'base1', v2: 'base2', a: 'xxx' }, 'a/b').buf!.length,
      6
    );
    assert.equal(
      tsb.encode({ v1: 'base1', abc: 'abc', v2: 'base2', a: 'xxx' }, 'a/b').buf!
        .length,
      15
    );
    assert.deepStrictEqual(
  ...
  it('Pick', async function () {
    ...
    [{ a: 'xxx' }, { a: 'xxx', c: { value: 'xxx' } }].forEach((v) => {
      ...
      assert.deepStrictEqual(
        tsb.decode(tsb.encode(v, 'a/b3').buf!, 'a/b3').value,
        v
      );
      assert.deepStrictEqual(
        tsb.decode(tsb.encode(v, 'a/b4').buf!, 'a/b4').value,
        v
      );
    });
    ...
    [{ a: 'xxx' }].forEach((v) => {
      assert.deepStrictEqual(
        tsb.decode(tsb.encode(v, 'a/b1').buf!, 'a/b1').value,
        v
      );
    });
  ...
  it('Pick keyof', async function () {
    ...
    [{ a: 'xxx' }, { a: 'xxx', c: { value: 'xxx' } }].forEach((v) => {
      ...
      assert.deepStrictEqual(
        tsb.decode(tsb.encode(v, 'a/b2').buf!, 'a/b2').value,
        v
      );
    });
  ...
  it('Overwrite', async function () {
    ...
    assert.equal(
      tsb.encode({ a: 'xxx', b: { value: 123 } }, 'a/b').buf!.length,
      11
    );
  ...
  it('AllowUnknownFields', async function () {
    ...
    [
      ...
      assert.deepStrictEqual(
        tsb.decode(tsb1.encode(v1, 'a/b').buf!, 'a/b').value,
        v
      );
      assert.deepStrictEqual(
        tsb.decode(tsb1.encode(v2, 'a/b').buf!, 'a/b').value,
        v
      );
    });
  });
...
export type NonNullable4 = NonNullable<Wrapper['value4']>;
    ...
    ['a/NonNullable1', 'a/NonNullable2', 'a/NonNullable3'].forEach(
      (schemaId) => {
        assert.deepStrictEqual(
          tsb.decode(tsb.encode('AAA', schemaId).buf!, schemaId).value,
          'AAA'
        );
      }
    );
    ...
    [
      ...
      assert.deepStrictEqual(
        tsb.decode(tsb.encode(v, 'a/NonNullable4').buf!, 'a/NonNullable4')
          .value,
        v
      );
    });
