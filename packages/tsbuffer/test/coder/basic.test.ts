...
describe('Basic Encode', function () {
  ...
  it('Number: uint', function () {
    ...
    assert.equal(tsb.encode(60, 'a/b').buf!.length, 1);
    assert.equal(
      tsb.encode(-60, 'a/b', { skipValidate: true }).buf!.length,
      10
    );
  ...
  it('Enum', async function () {
    ...
    [
      ...
      assert.strictEqual(
        tsb.decode(tsb.encode(v, 'a/TestEnum').buf!, 'a/TestEnum').value,
        v
      );
    });
  });
  ...
  it('Literal', function () {
    ...
    (
      ...
      assert.deepStrictEqual(
        tsb.decode(tsb.encode(v[1], 'a/' + v[0]).buf!, 'a/' + v[0]).value,
        v[1]
      );
    });
  });
  ...
  it('Buffer', function () {
    ...
    assert.deepStrictEqual(
      tsb.decode(tsb.encode(buf.buffer, 'a/b').buf!, 'a/b').value,
      buf.buffer
    );
    ...
    for (let key in typedArrays) {
      ...
      assert.equal(
        tsb.encode(arr1, 'a/b').buf!.length,
        1 + 10 * arrItem.itemByteLength
      );
      assert.deepStrictEqual(
        tsb.decode(tsb.encode(arr1, 'a/b').buf!, 'a/b').value,
        arr1,
        key + ' failed'
      );
      ...
      let wholeOri = Array.from({ length: 7 }, () => 0)
        .concat(value)
        .concat(Array.from({ length: 13 }, () => 0));
      ...
      // subarray 测试
      assert.equal(
        tsb.encode(arr, 'a/b').buf!.length,
        1 + 10 * arrItem.itemByteLength
      );
      assert.deepStrictEqual(
        tsb.decode(tsb.encode(arr, 'a/b').buf!, 'a/b').value,
        arr,
        key + ' failed'
      );
    }
