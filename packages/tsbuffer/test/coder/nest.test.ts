...
describe('Nest', function () {
  it('Array', async function () {
    ...
    [[], ['a/b', 'c']].forEach((v) => {
      assert.deepStrictEqual(
        tsb.decode(tsb.encode(v, 'a/c').buf!, 'a/c').value,
        v
      );
    });
    ...
    [[], [['a'], ['b', 'b'], ['c', 'c', 'c']]].forEach((v) => {
      assert.deepStrictEqual(
        tsb.decode(tsb.encode(v, 'a/d').buf!, 'a/d').value,
        v
      );
    });
  });
  ...
  it('Tuple', async function () {
    ...
    assert.deepStrictEqual(
      tsb.decode(
        tsb.encode([123.124, 'asdg', undefined, undefined], 'a/c').buf!,
        'a/c'
      ).value,
      [123.124, 'asdg']
    );
    assert.deepStrictEqual(
      tsb.decode(
        tsb.encode([123.124, 'asdg', undefined, 123, undefined], 'a/c').buf!,
        'a/c'
      ).value,
  ...
  it('Cycle Reference in interface', async function () {
    ...
    assert.deepStrictEqual(
      tsb.decode(tsb.encode(root, 'a/Node').buf!, 'a/Node').value,
      root
    );
  });
});
