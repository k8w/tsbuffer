...
describe('LogicTypes', function () {
  it('A | B', async function () {
    ...
    assert.deepStrictEqual(
      tsb.decode(tsb.encode(null, 'a/b2').buf!, 'a/b2').value,
      null
    );
    assert.deepStrictEqual(
      tsb.decode(tsb.encode([null], 'a/arr').buf!, 'a/arr').value,
      [null]
    );
  });
  ...
  it('A & B', async function () {
    ...
    assert.equal(
      tsb.encode({ a: 'abc', b: 123, c: 'asdfasdf' }, 'a/b').buf!.length,
      21
    );
  ...
  it('Unknown Union ID', async function () {
    ...
    [
      ...
      assert.deepStrictEqual(
        tsb.decode(tsb.encode(v, 'a/a').buf!, 'a/a1').value,
        v
      );

      let v1 = Object.assign({}, v, { d: [true, false] });
      assert.deepStrictEqual(
        tsb.decode(tsb.encode(v1, 'a/a1').buf!, 'a/a').value,
        v
      );
    });
  });
  ...
  it('Unknown Intersection ID', async function () {
    ...
    assert.deepStrictEqual(
      tsb.decode(tsb.encode(v, 'a/a1').buf!, 'a/a').value,
      v
    );
    assert.deepStrictEqual(
      tsb.decode(tsb.encode(v1, 'a/a1').buf!, 'a/a').value,
