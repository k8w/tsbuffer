...
describe('Interface', function () {
  it('property', async function () {
    let proto = await new TSBufferProtoGenerator({
      readFile: () => `
                export interface b {
                    ...
                    e?: {a:'${'0'.repeat(1000)}', b: string} | {a:'${'1'.repeat(
        1000
      )}', b: number},
                    f?: {a:string} & {b:string}
                }
    ...
    [
      ...
      assert.deepStrictEqual(
        tsb.decodeJSON(
          JSON.parse(JSON.stringify(tsb.encodeJSON(v, 'a/b').json!)),
          'a/b'
        ).value,
        v
      );
    ...
    assert.deepStrictEqual(
      tsb.decodeJSON(
        JSON.parse(
          JSON.stringify(
            tsb.encodeJSON({ a: 'xxxx', b: undefined }, 'a/b').json!
          )
        ),
        'a/b'
      ).value,
  ...
  it('完整性检查', async function () {
    ...
    let buf = JSON.parse(
      JSON.stringify(tsb1.encodeJSON({ a: 'aaa' }, 'a/b').json!)
    );
  ...
  it('Pick/Omit加字段，Decoder协议未更新仍旧不读', async function () {
    ...
    let bufB1 = JSON.parse(
      JSON.stringify(tsb.encodeJSON({ a: 'aaa', b: 'bbb' }, 'a/b1').json!)
    );
    let bufB2 = JSON.parse(
      JSON.stringify(tsb.encodeJSON({ a: 'aaa', b: 'bbb' }, 'a/b2').json!)
    );
  ...
  it('Pick', async function () {
    ...
    [{ a: 'xxx' }].forEach((v) => {
      assert.deepStrictEqual(
        tsb.decodeJSON(
          JSON.parse(JSON.stringify(tsb.encodeJSON(v, 'a/b1').json!)),
          'a/b1'
        ).value,
        v
      );
  ...
  it('Pick keyof', async function () {
    ...
    [{ a: 'xxx' }, { a: 'xxx', c: { value: 'xxx' } }].forEach((v) => {
      assert.deepStrictEqual(
        tsb.decodeJSON(tsb.encodeJSON(v, 'a/b').json!, 'a/b').value,
        v
      );
      assert.deepStrictEqual(
        tsb.decodeJSON(tsb.encodeJSON(v, 'a/b1').json!, 'a/b1').value,
        v
      );
      assert.deepStrictEqual(
        tsb.decodeJSON(tsb.encodeJSON(v, 'a/b2').json!, 'a/b2').value,
        v
      );
    });
  ...
  it('Overwrite', async function () {
    ...
    [
      ...
      assert.deepStrictEqual(
        tsb.decodeJSON(
          JSON.parse(JSON.stringify(tsb.encodeJSON(v, 'a/b2').json!)),
          'a/b2'
        ).value,
        v
      );
  ...
  it('AllowUnknownFields', async function () {
    ...
    [
      ...
      assert.deepStrictEqual(
        tsb.decodeJSON(
          JSON.parse(JSON.stringify(tsb1.encodeJSON(v1, 'a/b').json!)),
          'a/b'
        ).value,
        v
      );
      assert.deepStrictEqual(
        tsb.decodeJSON(
          JSON.parse(JSON.stringify(tsb1.encodeJSON(v2, 'a/b').json!)),
          'a/b'
        ).value,
        v
      );
...
export type NonNullable4 = NonNullable<Wrapper['value4']>;
    ...
    ['a/NonNullable1', 'a/NonNullable2', 'a/NonNullable3'].forEach(
      (schemaId) => {
        assert.deepStrictEqual(
          tsb.decodeJSON(
            JSON.parse(JSON.stringify(tsb.encodeJSON('AAA', schemaId).json!)),
            schemaId
          ).value,
          'AAA'
        );
