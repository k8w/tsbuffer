...
describe('Nest', function () {
  it('Array', async function () {
    ...
    [[], ['a/b', 'c']].forEach((v) => {
      assert.deepStrictEqual(
        tsb.decodeJSON(
          JSON.parse(JSON.stringify(tsb.encodeJSON(v, 'a/c').json!)),
          'a/c'
        ).value,
        v
      );
    ...
    [[], [['a'], ['b', 'b'], ['c', 'c', 'c']]].forEach((v) => {
      assert.deepStrictEqual(
        tsb.decodeJSON(
          JSON.parse(JSON.stringify(tsb.encodeJSON(v, 'a/d').json!)),
          'a/d'
        ).value,
        v
      );
  ...
  it('Tuple', async function () {
    ...
    assert.deepStrictEqual(
      tsb.decodeJSON(
        JSON.parse(
          JSON.stringify(
            tsb.encodeJSON([123.124, 'asdg', undefined, undefined], 'a/c').json!
          )
        ),
        'a/c'
      ).value,
