...
describe('Utf8Coder', function () {
  ...
  it('encode & decode', function () {
    ...
    assert.strictEqual(
      ...
      Buffer.from(nodeBuf.buffer, nodeBuf.byteOffset, nodeBuf.byteLength).write(
        str,
        5,
        'utf-8'
      )
    ...
    assert.strictEqual(
      Buffer.from(
        ...
        nodeBuf.byteOffset,
        nodeBuf.byteLength
      ).toString('utf-8', 5, 5 + jsLength),
      str
    );
    assert.strictEqual(
      Buffer.from(jsBuf.buffer, jsBuf.byteOffset, jsBuf.byteLength).toString(
        'utf-8',
        5,
        5 + jsLength
      ),
