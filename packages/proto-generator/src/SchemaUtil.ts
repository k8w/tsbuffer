...
export class SchemaUtil {
   ...
   * @param schema
   */
  static getUsedReferences(
    schemas: TSBufferSchema | TSBufferSchema[]
  ): ReferenceTypeSchema[] {
    ...
    for (const schema of schemas) {
      switch (schema.type) {
        ...
        case SchemaType.Interface:
          if (schema.extends) {
            output = output.concat(
              this.getUsedReferences(schema.extends.map((v) => v.type))
            );
          }
          if (schema.properties) {
            output = output.concat(
              this.getUsedReferences(schema.properties.map((v) => v.type))
            );
          }
          if (schema.indexSignature) {
            output = output.concat(
              this.getUsedReferences(schema.indexSignature.type)
            );
          }
          break;
        ...
        case SchemaType.Intersection:
          output = output.concat(
            this.getUsedReferences(schema.members.map((v) => v.type))
          );
          break;
        ...
        case SchemaType.Omit:
          ...
          if ((schema as PrePickOmitSchema).pre?.key) {
            output = output.concat(
              this.getUsedReferences((schema as PrePickOmitSchema).pre.key)
            );
          }
          break;
