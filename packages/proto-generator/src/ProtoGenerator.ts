...
export class ProtoGenerator {
  protected readonly options: ProtoGeneratorOptions = {
    ...
    verbose: false,
    readFile: (v) =>
      fs.readFileSync(path.resolve(this.options.baseDir, v)).toString(),
  ...
  async generate(
    ...
    paths = paths.map((v) =>
      path.relative(this.options.baseDir, path.resolve(this.options.baseDir, v))
    );
    ...
    if (this.options.verbose) {
      logger?.debug(
        ...
        'generate',
        `Ready to generate ${paths.length} file`
      );
      logger?.debug(
        '[TSBuffer Schema Generator]',
        'generate',
        ...
        if (
          ...
          if (this.options.verbose) {
            logger?.debug(
              '[TSBuffer Schema Generator]',
              'generate',
              `filter passed: ${name} at ${filepath}`
            );
          }
          ...
          logger?.debug(
            '[TSBuffer Schema Generator]',
            'generate',
            `filter not passed: ${name} at ${filepath}`
          );
        }
      }
    }
  ...
  private _regenSchemaEncodeIds(
    ...
    compatibleSchema?: TSBufferSchema
  ) {
    // 不仅要有 还要是同类型才行
    ...
    switch (schema.type) {
      ...
      case 'Interface': {
        ...
        if (schema.extends) {
          const cpExtends =
            compatibleSchema &&
            (compatibleSchema as InterfaceTypeSchema).extends;
          const cpIds =
            cpExtends &&
            cpExtends.map((v) => ({
          ...
          const ids = EncodeIdUtil.genEncodeIds(
            schema.extends.map((v) => JSON.stringify(v.type)),
            cpIds
          );
          for (let i = 0; i < ids.length; ++i) {
        ...
        if (schema.properties) {
          ...
          const cpSchemaProps =
            compatibleSchema &&
            (compatibleSchema as InterfaceTypeSchema).properties;
          for (let i = 0; i < ids.length; ++i) {
            // 更新ID
            ...
            const subCpProp =
              cpSchemaProps &&
              cpSchemaProps.find((v) => v.name === schema.properties![i].name);
            this._regenSchemaEncodeIds(
              schema.properties[i].type,
              subCpProp ? subCpProp.type : undefined
            );
          }
        }
        ...
        if (schema.indexSignature) {
          const cpIndexSignature =
            (compatibleSchema &&
              (compatibleSchema as InterfaceTypeSchema).indexSignature &&
              (compatibleSchema as InterfaceTypeSchema).indexSignature!.type) ||
            undefined;
          this._regenSchemaEncodeIds(
      ...
      case 'Union':
        ...
        for (let i = 0; i < ids.length; ++i) {
          ...
          const subCpMember =
            compatibleSchema &&
            (
              compatibleSchema as IntersectionTypeSchema | UnionTypeSchema
            ).members.find((v) => v.id === ids[i].id);
      ...
      case 'Array':
        ...
        this._regenSchemaEncodeIds(
          schema.elementType,
          compatibleSchema && (compatibleSchema as ArrayTypeSchema).elementType
        );
        break;
      case 'IndexedAccess':
        this._regenSchemaEncodeIds(
          schema.objectType,
          compatibleSchema &&
            (compatibleSchema as IndexedAccessTypeSchema).objectType
        );
        break;
      case 'Tuple':
        for (let i = 0; i < schema.elementTypes.length; ++i) {
          this._regenSchemaEncodeIds(
            ...
            compatibleSchema &&
              (compatibleSchema as TupleTypeSchema).elementTypes[i]
          );
        }
        break;
    }
  ...
  private async _getAst(
    ...
    if (!astCache[astKey]) {
      ...
      if (fileContent === undefined) {
        throw new Error(
          `Cannot resolve file: ` + path.resolve(this.options.baseDir, astKey)
        );
      }
   ...
   * @returns
   */
  private async _addRefToOutput(
    ref: ReferenceTypeSchema,
    astKey: string,
    name: string,
    output: TSBufferProto,
    astCache: AstCache,
    ...
    if (this.options.customSchemaIds?.includes(ref.target)) {
      if (this.options.verbose) {
        logger?.debug(
          '[TSBuffer Schema Generator]',
          `Ignored Reference Target '${ref.target}'`
        );
      }
    ...
    if (pathMatch) {
      ...
      // 绝对路径引用 resolveModule
        ...
        if (!this.options.resolveModule) {
          throw new Error(
            `Must specific a resolveModule handler for resolve '${pathMatch[1]}'`
          );
        }
        refPath = path.relative(
          this.options.baseDir,
          this.options.resolveModule(pathMatch[1])
        );
      }
    }
    // 当前文件内引用
    ...
    if (this.options.verbose) {
      logger?.debug(
        '[TSBuffer Schema Generator]',
        `addToOutput(${astKey}, ${name}})`,
        `refTargetName=${certainRefTargetName}`
      );
    }
    ...
    if (certainRefTargetName) {
      ...
      await this._addToOutput(
        refAst.astKey,
        certainRefTargetName,
        refAst.ast[certainRefTargetName].schema,
        output,
        astCache,
        logger
      );
        ...
        await this._addRefToOutput(
          {
            ...
            target: ref.target.substr(
              0,
              ref.target.length - rMatch[2].length - 1
            ),
          },
          astKey,
          name,
    ...
    throw new Error(
      `Cannot find reference target '${ref.target}'\n    at ${name}\n    at ${astKey}`
    );
  }
  ...
  private _calcPreKey(schema: PreSchema, logger: Logger | undefined): string[] {
    // Nested pre key
    if (
      (schema.type === SchemaType.Pick || schema.type === SchemaType.Omit) &&
      schema.pre
    ) {
    ...
    if (this._astParser['_isTypeReference'](schema)) {
      return this._calcPreKey(
        this._postHelper.parseReference(schema) as PreSchema,
        logger
      );
    }
    ...
    switch (schema.type) {
      case SchemaType.Union:
        return schema.members
          .map((v) => this._calcPreKey(v.type as PreSchema, logger))
          .reduce((prev, next) => prev.concat(next), [])
          .distinct();
      case SchemaType.Intersection:
        return schema.members
          .map((v) => this._calcPreKey(v.type as PreSchema, logger))
          .reduce((prev, next) => prev.filter((v) => next.indexOf(v) > -1));
...
export const defaultResolveModule: NonNullable<
  ProtoGeneratorOptions['resolveModule']
